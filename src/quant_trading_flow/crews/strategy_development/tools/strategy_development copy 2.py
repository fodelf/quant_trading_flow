...
class StockPredictor:
    ...
    def objective_change(
        self, trial, train_valid_data, y_train_high, df, org_df, test_size
    ):
        ...
        # 修复1: 使用更有意义的变量名，避免使用内置函数名
        feature_list = [s for s in self.feature_columns if s != "Change"]

        # 修复2: 确保选择的特征列确实存在于DataFrame中
        available_columns = [
            col for col in feature_list if col in train_valid_data.columns
        ]

        if not available_columns:
            raise ValueError(
                f"没有可用的特征列用于训练。尝试从{feature_list}中选择，但train_valid_data中只有这些列: {list(train_valid_data.columns)}"
            )

        X_train = train_valid_data[available_columns]
        ...
        for i in range(start_index, end_index):
            ...
            X_test = test_data[available_columns]
    ...
    def objective(
        self, trial, train_valid_data, y_train_high, df, org_df, test_size, key
    ):
        """Optuna目标函数，用于优化LightGBM参数"""
        # 参数建议范围
        params = {
        ...
        # # 修复1: 使用更有意义的变量名，避免使用内置函数名
        # feature_list = [s for s in self.feature_columns if s != key]

        # # 修复2: 确保选择的特征列确实存在于DataFrame中
        # available_columns = [
        #     col for col in feature_list if col in train_valid_data.columns
        # ]
        # print(f"available_columns11111111111")
    ...
    def train(self, train_valid_data, y_train, df, org_df, test_data, key):
        ...
        # list = [s for s in self.feature_columns if s != key]
        X_train = train_valid_data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
    ...
    def walk_forward_backtest(self, df):
        ...
        self.model_high = self.train(
            train_valid_data, y_train_high, df, org_df, test_data, "High"
        )
        self.model_low = self.train(
            train_valid_data, y_train_low, df, org_df, test_data, "Low"
        )
        self.model_close = self.train(
            train_valid_data, y_train_close, df, org_df, test_data, "Close"
        )
        self.model_open = self.train(
            train_valid_data, y_train_open, df, org_df, test_data, "Open"
        )
        self.model_volume = self.train(
            train_valid_data, y_train_volume, df, org_df, test_data, "Volume"
        )
        self.model_amount = self.train(
            train_valid_data, y_train_amount, df, org_df, test_data, "Amount"
        )
        self.model_amplitude = self.train(
            train_valid_data, y_train_amplitude, df, org_df, test_data, "Amplitude"
        )
        self.model_turnoverRate = self.train(
            train_valid_data,
            y_train_turnoverRate,
            df,
            org_df,
            test_data,
            "TurnoverRate",
        )
        self.model_changeAmount = self.train(
            train_valid_data,
            y_train_changeAmount,
            df,
            org_df,
            test_data,
            "ChangeAmount",
        ...
        # list = [s for s in self.feature_columns if s != "Change"]
        X_train = train_valid_data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        study = optuna.create_study(direction="minimize")  # 最小化RMSE
        study.optimize(
            lambda trial: self.objective_change(
                trial, train_valid_data, y_train_change, df, org_df, len(test_data)
            ),
            n_trials=n_trials,
        )
        trial = study.best_trial
        ...
        study.optimize(
            lambda trial: self.objective_direction(
                trial, train_valid_data, y_train_direction, df, org_df, len(test_data)
            ),
            n_trials=n_trials,
        )  # 尝试100组参数组
        trial = study.best_trial
    ...
    def predict_day(self, data):
        ...
        list = [s for s in self.feature_columns if s != "High"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        # 进行预测
        pred_high = self.model_high.predict(X_scaled)[0]

        list = [s for s in self.feature_columns if s != "Low"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        pred_low = self.model_low.predict(X_scaled)[0]

        list = [s for s in self.feature_columns if s != "Close"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        ...
        list = [s for s in self.feature_columns if s != "Open"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        pred_open = self.model_open.predict(X_scaled)[0]

        list = [s for s in self.feature_columns if s != "TurnoverRate"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        pred_turnoverRate = self.model_turnoverRate.predict(X_scaled)[0]

        list = [s for s in self.feature_columns if s != "ChangeAmount"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        pred_changeAmount = self.model_changeAmount.predict(X_scaled)[0]

        list = [s for s in self.feature_columns if s != "Volume"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        pred_volume = self.model_volume.predict(X_scaled)[0]

        list = [s for s in self.feature_columns if s != "Amount"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
        pred_amount = self.model_amount.predict(X_scaled)[0]

        list = [s for s in self.feature_columns if s != "Amplitude"]
        X_train = data[self.feature_columns]
        X_scaled = self.scaler.fit_transform(X_train)
