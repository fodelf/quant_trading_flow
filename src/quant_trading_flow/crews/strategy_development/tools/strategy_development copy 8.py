...
class StockPredictor:
    ...
    def handle_missing_values(self, df):
        ...
        for col in price_cols:
            if col in df_filled.columns:
                ...
                # 如果还有缺失，使用最近的有效值
                df_filled[col] = (
                    df_filled[col].fillna(method="ffill").fillna(method="bfill")
                )
    ...
    def prepare_features_and_targets(self, df):
        ...
        # df_processed["target_开盘"] = df_processed["开盘"].shift(-1)
        # df_processed["target_收盘"] = df_processed["收盘"].shift(-1)
        # df_processed["target_成交量"] = df_processed["成交量"].shift(-1)
        # df_processed["target_最高"] = df_processed["最高"].shift(-1)
        # df_processed["target_最低"] = df_processed["最低"].shift(-1)

        # 删除包含NaN的行
        df_processed = df_processed.dropna()

        # 动态确定要排除的列，只排除实际存在的列
        exclude_columns = ["Date", "股票代码"]
        # 只排除实际存在于数据框中的列
        existing_exclude_columns = [
            col
            for col in ["开盘", "最高", "最低", "成交量"]
            if col in df_processed.columns
        ]
        exclude_columns.extend(existing_exclude_columns)
    ...
    def prepare_features_and_targets_next(self, df):
        """
        准备特征和目标变量
        """
        df_processed = df.copy()
        ...
        df_processed["target_volume"] = df_processed["Volume"].shift(-2)
        df_processed["target_amount"] = df_processed["Amount"].shift(-2)
        df_processed["target_amplitude"] = df_processed["Amplitude"].shift(-2)
        df_processed["target_turnoverRate"] = df_processed["TurnoverRate"].shift(-2)
        df_processed["target_changeAmount"] = df_processed["ChangeAmount"].shift(-2)

        # df_processed["target_开盘"] = df_processed["开盘"].shift(-2)
        # df_processed["target_收盘"] = df_processed["收盘"].shift(-2)
        # df_processed["target_成交量"] = df_processed["成交量"].shift(-2)
        # df_processed["target_最高"] = df_processed["最高"].shift(-2)
        # df_processed["target_最低"] = df_processed["最低"].shift(-2)
    ...
    def objective(
        self, trial, X_scaled, y_train_high, df, org_df, test_size, key, shift
    ):
        """Optuna目标函数，用于优化LightGBM参数"""
        # 参数建议范围
        params = {
        ...
        scores = []
        for i in range(start_index, end_index):
            test_data = org_df.iloc[i : i + shift]
            X_test = test_data[self.feature_columns]
            X_test_scaled = self.scaler.transform(X_test)
            pred_high = model.predict(X_test_scaled)[0]
            true_high = org_df.iloc[i + shift][key]
            all_high_preds.append(pred_high)
            all_high_true.append(true_high)
        # 预测并计算RMSE
        rmse = np.sqrt(mean_squared_error(all_high_true, all_high_preds))
    ...
    def tran_model(self, X_scaled, y_train, df, org_df, test_data, key, shift):
        n_jobs = multiprocessing.cpu_count() - 1
        study = optuna.create_study(direction="minimize")
        study.optimize(
            lambda trial: self.objective(
                trial, X_scaled, y_train, df, org_df, len(test_data), key, shift
            ),
            n_trials=n_trials,
            n_jobs=n_jobs,
        )  # 尝试100组参数组
    ...
    # 定义目标函数
    # 3. 定义Optuna目标函数
    def walk_forward_backtest(self, df, next, fit=False):
        """
        ...
        # y_train_开盘 = train_valid_data["target_开盘"]
        # y_train_收盘 = train_valid_data["target_收盘"]
        # y_train_成交量 = train_valid_data["target_成交量"]
        # y_train_最高 = train_valid_data["target_最高"]
        # y_train_最低 = train_valid_data["target_最低"]
        ...
        # 创建Optuna研究并优化
        # self.model_high = self.tran_model(
        #     X_scaled, y_train_high, df, org_df, test_data, "High"
        # )
        if next:
            ...
            # )
            # self.model_close = self.tran_model(
            #     X_scaled, y_train_close, df, org_df, test_data, "Close"
            # )
            # self.model_change = self.tran_model(
            #     X_scaled, y_train_change, df, org_df, test_data, "Change"
            # )
            # self.model_开盘 = self.tran_model(
            #     X_scaled, y_train_开盘, df, org_df, test_data, "Open"
            # )
            # self.model_收盘 = self.tran_model(
            #     X_scaled, y_train_收盘, df, org_df, test_data, "Close"
            # )
            # self.model_成交量 = self.tran_model(
            #     X_scaled, y_train_成交量, df, org_df, test_data, "Volume"
            # )
            # self.model_最高 = self.tran_model(
            #     X_scaled, y_train_最高, df, org_df, test_data, "High"
            # )
            # self.model_最低 = self.tran_model(
            #     X_scaled, y_train_最低, df, org_df, test_data, "Low"
            # )
            self.model_low = self.tran_model(
                X_scaled, y_train_low, df, org_df, test_data, "Low", 1
            )
            # print("------")
        else:
            self.model_low = self.tran_model(
                X_scaled, y_train_low, df, org_df, test_data, "Low", 2
            )
            self.model_high = self.tran_model(
                X_scaled, y_train_high, df, org_df, test_data, "High", 2
            )
            n_jobs = multiprocessing.cpu_count() - 1
    ...
    def predict_day_next(self, data):
        """
        预测指定日期的数据
        """
        # 准备特征数据
        ...
        # pred_开盘 = self.model_开盘.predict(X_scaled)[0]
        # pred_收盘 = self.model_收盘.predict(X_scaled)[0]
        # pred_成交量 = self.model_成交量.predict(X_scaled)[0]
        # pred_最高 = self.model_最高.predict(X_scaled)[0]
        # pred_最低 = self.model_最低.predict(X_scaled)[0]
        ...
        return {
            ...
            # "predicted_开盘": pred_开盘,
            # "predicted_收盘": pred_收盘,
            # "predicted_成交量": pred_成交量,
            # "predicted_最高": pred_最高,
            # "predicted_最低": pred_最低,
...
def formatData(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    # df = df.drop(columns=["开盘", "最高", "最低", "成交量"])
    # # 基础价格特征
    ...
    # df["amount_per_volume"] = df["Amount"] / (df["Volume"] + 1e-8)
    # # 历史涨停情况
    # df["prev_limit_up"] = ((df["Close"] / df["Close"].shift(1) - 1) > 0.095).astype(int)
    # df["limit_up_count_5"] = df["prev_limit_up"].rolling(5).sum()
    # df["limit_up_count_10"] = df["prev_limit_up"].rolling(10).sum()

    # # 距离涨停板的距离
    # df["distance_to_limit_up"] = (df["Close"] * 1.1 - df["Close"]) / df["Close"] * 100
    # df["distance_to_limit_down"] = (df["Close"] - df["Close"] * 0.9) / df["Close"] * 100
    ...
    #     else:
    #         consecutive = 0
    #     df.loc[df.index[i], "consecutive_up_days"] = consecutive
    # # for i in range(1, 6):  # 使用前5天数据
...
def run_strategy_development_action(symbol, file_date):
    ...
    fit = False
    if latest_pred["predicted_low"] < low_value_iloc:
        fit = True
        backtest_results = predictor.walk_forward_backtest(df_processed, True, fit)
        latest_pred = predictor.predict_day(latest_data)
    # 9. 输出结果
    result = {
        "symbol": symbol,
    ...
    # newLastData["开盘"] = latest_pred["predicted_开盘"]
    # newLastData["收盘"] = latest_pred["predicted_收盘"]
    # newLastData["成交量"] = latest_pred["predicted_成交量"]
    # newLastData["最高"] = latest_pred["predicted_最高"]
    # newLastData["最低"] = latest_pred["predicted_最低"]
    # df = pd.concat([df, newLastData], ignore_index=True)
    predictor.df = formatData(df)
    df_processed = predictor.prepare_features_and_targets_next(df)
    predictor.df_processed = df_processed  # 保存处理后的数据供后续使用
    print(f"特征工程完成，剩余 {len(df_processed)} 条有效记录")
    ...
    # 4. 执行滚动窗口回测
    # print("执行滚动窗口回测...")
    backtest_results = predictor.walk_forward_backtest(df_processed, False, fit)
    latest_data = df.iloc[-1:]  # 获取最新数据
    latest_pred = predictor.predict_day_next(latest_data)
    result = {
        "symbol": symbol,
        "model_used": "LightGBM",
        "features": predictor.feature_columns,
