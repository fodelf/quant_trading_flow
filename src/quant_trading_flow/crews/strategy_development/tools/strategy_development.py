...
class StockPredictor:
    ...
    def prepare_features_and_targets(self, df):
        ...
        # 定义目标变量 - 当天数据
        df_processed["target_high"] = df_processed["High"]
        df_processed["target_low"] = df_processed["Low"]
    ...
    def walk_forward_backtest(self, df):
        ...
        for i in range(start_index, end_index):
            ...
            # 进行预测
            pred_high = model_high.predict(X_test_scaled)[0]
            ...
            # 获取真实值
            true_high = df.iloc[i]["target_high"]
...
# 主函数
def run_strategy_development_action(symbol, data_path, target_dates=None):
    """
    主函数：执行完整的策略评估流程
    ...
    # 7. 预测最新日期
    print("预测最新日期...")
    latest_data = df_processed.iloc[-1:]  # 获取最新数据
...
# 使用示例
def run_strategy_development(symbol, data_path, target_dates):
    setup_chinese_font()
    # # 执行策略评估
    result = run_strategy_development_action(symbol, data_path, target_dates)
